...
test('getCotacaoAPI', () => {
    ...
    api.getCotacaoAPI('url').then( resp => {
        expected(res).toEqual(res)
        expected.axios.get.mock.calls[0][0].toBe('url')
    })
})
...
test('getUrl', () => {
    ...
    expect(url).toBe(`https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/CotacaoDolarDia(dataCotacao=@dataCotacao)?@dataCotacao='MINHA-DATA'&$top=100&$format=json&$select=cotacaoCompra,cotacaoVenda`)
})
